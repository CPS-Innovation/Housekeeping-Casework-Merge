  ...
  return getPackageNamesInOrder()
    .reduce((accum, packageName) => Object.assign({}, accum, objectMap(
      ...
      (listOfItemsForType, type) => (accum[type] || [])
        .concat([].concat(listOfItemsForType)
          // govuk-frontend v5+ uses object entries {path, type} for scripts.
          // The v4 prototype-kit framework only handles string paths, so skip
          // any non-string items to prevent a crash.
